        ...
        while not done and keep_going(steps, num_steps, episodes,
            ...
            for agent_id, a_obs in multi_obs.items():
                if a_obs is not None:
                    original_shape = [64,64,6]
                    obs = a_obs.astype(np.int32)
                    subtract_mask = np.zeros(list(original_shape))
                    subtract_mask[...,3:] = 1
                    subtract_part = subtract_mask * obs
                    subtract_part = subtract_part * 2 - 255
                    obs = obs * (1-subtract_mask) + subtract_part * subtract_mask
                    i+=1
                    plt.imshow(obs[...,:3].astype(np.uint8))
                    plt.savefig(f'images/a{i}.png')
                    plt.imshow(np.abs(obs[...,3:]).astype(np.uint8))
                    plt.savefig(f"images/b{i}.png")
                    if i>5:
                        exit(1)
