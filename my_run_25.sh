...
if [[ " $@ " =~ " --train " ]]; then
  ...
  python train_tune.py -f ${EXPERIMENT} --env-name=${ENVIRONMENT_NAME} --num_levels=200 --num-workers=4 --num-cpus-per-worker=0.25 --ray-memory ${RAY_MEMORY_LIMIT:-60129542144} --ray-num-cpus ${RAY_CPUS:-2} --ray-object-store-memory ${RAY_STORE_MEMORY:-3000000000}
  STATUS_CODE=$?
fi
...
if [[ " $@ " =~ " --rollout " ]]; then
  ...
  python ./rollout_render.py $CHECKPOINT --episodes ${EPISODES:-1000} --run $ROLLOUT_RUN --exploration=1 --distribution-mode=easy --num_levels=0 --temperature=0.25
  STATUS_CODE=$?
fi
